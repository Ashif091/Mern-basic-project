...
const UserData = () => {
  ...
  const [UformData, UsetFormData] = useState({})
  const [error, setError] = useState(false)
  const [update, setUpadate] = useState(false)
  const nameInput = useRef<any>()
  const emailnput = useRef<any>()
  const idlnput = useRef<any>()

  const handleChange = (e: any) => {
    setFormData({...formData, [e.target.id]: e.target.value})
  }
  const UpdateChange = (e: any) => {
    UsetFormData({...UformData, [e.target.id]: e.target.value})
  }
  ...
  const updateForm = async (data: any) => {
    ...
    emailnput.current.value = data.email
    idlnput.current.value = data._id
  }
  ...
  return (
    <div className="w-full h text-4xl flex gap-0 ">
      ...
      {/* user edit form  */}
      <form
        onSubmit={chageSubmit}
        className={`${
          update ? "" : "hidden"
        } max-w-sm max-h-52 absolute left-24 top-12 mx-auto border border-gray-600 mt-20 pt-2 px-10 rounded-md items-center text-center bg-gray-700 shadow-[rgba(0,_0,_0,_0.2)_0px_60px_40px_-7px]`}
      >
        <h1 className="text-xl mb-2 text-gray-400">Edit User</h1>
        <div className="flex">
          <span className="inline-flex items-center px-3 text-sm text-gray-900 bg-gray-200 border border-e-0 border-gray-300 rounded-s-md dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
            <svg
              className="w-4 h-4 text-gray-500 dark:text-gray-400"
        ...
        <div className="flex justify-around mt-2">
          <div
            ...
            className="text-gray-900 hover:text-white  pt-2 cursor-pointer  text-sm   text-center me-2 mb-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white  dark:focus:ring-gray-800"
          >
            Cancel
          </div>
          <button className="text-blue-900 hover:text-white border border-gray-800 hover:bg-blue-900 focus:ring-1 focus:outline-none focus:ring-gray-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-blue-800">
            ADD
          </button>
        </div>
      </form>
              ...
              <li className="pb-3 sm:pb-4" key={index}>
                <div className="flex items-center space-x-4 rtl:space-x-reverse">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate dark:text-white flex justify-between max-w-36 ">
                      <p> {user.username}</p>
